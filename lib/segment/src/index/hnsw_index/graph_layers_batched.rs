...
impl<S: UniversalRead> GraphLayersBatched<S> {
    ...
    /// Batched-IO counterpart of [`GraphLayers::search`].
    ...
    /// Counterpart of [`GraphLayers::search_entry`].
    fn search_entry(
        &self,
        entry_point: EntryPoint,
        target_level: usize,
    ...
    /// Counterpart of [`super::graph_layers::GraphLayersWithVectors::search_entry_with_vectors`].
    #[allow(clippy::too_many_arguments)]
            ...
            while changed {
                ...
                self.links.links_with_vectors(
                    ...
                    |_, _base_vector, links_iter, link_vectors| {
                        let mut points: Vec<(PointOffsetType, &[u8])> =
                            links_iter.zip(link_vectors).collect();
                        links_scorer
    ...
    fn search_on_level_with_vectors(
        ...
        loop {
            ...
            // links), and the search stops after this round.
            ...
            self.links.links_with_vectors(
                ...
                |position, base_vector, links_iter, link_vectors| {
                    ...
                    let mut points: Vec<(PointOffsetType, &[u8])> = links_iter
                        .zip(link_vectors)
                        .filter(|&(link, _)| !visited_list.check(link))
                        .collect();
                    links_scorer
                        .score_points(&mut points, limit)
                        .for_each(|score_point| {
    ...
    /// Batched version of [`super::graph_layers::GraphLayersBase::search_on_level`].
    #[allow(clippy::too_many_arguments)]
    fn search_on_level(
        &self,
        level_entry: ScoredPointOffset,
        ...
        loop {
            ...
            let n_matches = filters.check_batched(&mut links, Select::Matches, Rest::Discard)?;

            quotas.resize(batch.len(), member_limit);
            for &link in &links[..n_matches] {
    ...
    /// Batched version of [`super::graph_layers::GraphLayersBase::search_on_level_acorn`].
...
mod tests {
    ...
    /// node), with-vectors results come only from *expanded* nodes, so large
    /// batch sizes expand a different set and are not compared.
    #[test]
