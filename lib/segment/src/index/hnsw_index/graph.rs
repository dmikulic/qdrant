...
impl<S: UniversalRead> HnswGraph<S> {
        ...
        S: 'static,
    {
        Ok(if Self::is_batched(fs, dir, residency)? {
            ...
            HnswGraph::InRam(GraphLayers::load_universal(fs, dir, residency)?)
        })
    }
    ...
    pub fn search(&self, args: GraphSearchArgs<'_>) -> OperationResult<Vec<ScoredPointOffset>> {
        ...
        match (self, &mut scorers) {
            (HnswGraph::InRam(graph), SearchScorers::Regular(scorer)) => {
                Ok(graph.search(top, ef, algorithm, scorer, entry, is_stopped)?)
            }
            (HnswGraph::InRam(graph), SearchScorers::WithVectors(scorers)) => {
    ...
    fn point_level(&self, point_id: PointOffsetType) -> OperationResult<usize> {
        match self {
            HnswGraph::InRam(graph) => Ok(graph.point_level(point_id)),
    ...
    fn entry_points(&self) -> &EntryPoints {
        match self {
            HnswGraph::InRam(graph) => &graph.entry_points,
    ...
    pub fn has_inline_vectors(&self) -> bool {
        match self {
            HnswGraph::InRam(graph) => graph.links.format().is_with_vectors(),
            HnswGraph::Batched(graph) => graph.links.is_with_vectors(),
        }
    }

    pub(super) fn graph_layers(&self) -> &GraphLayers {
        match self {
            HnswGraph::InRam(graph) => graph,
            HnswGraph::Batched(_) => unreachable!("the writable HNSW index graph is mmap-backed"),
        }
    }

    pub fn num_points(&self) -> usize {
        match self {
            HnswGraph::InRam(graph) => graph.links.num_points(),
    ...
    pub fn files(&self, path: &Path) -> Vec<PathBuf> {
        match self {
            HnswGraph::InRam(graph) => graph.files(path),
    ...
    pub fn links_heap_size_bytes(&self) -> usize {
        match self {
            HnswGraph::InRam(graph) => graph.links_heap_size_bytes(),
    ...
    pub fn populate(&self) -> OperationResult<()> {
        match self {
            HnswGraph::InRam(graph) => graph.links.populate(),
    ...
    pub fn clear_cache(&self) -> OperationResult<()> {
        match self {
            HnswGraph::InRam(graph) => graph.links.clear_cache(),
